...
### Per-Model Weight Loading Status (updated 2026-08-18; supersedes the 2026-07-09 breakdown below, which is stale on 7 of its architectures)
- **Complete (53/55 feature-gated architectures)**: the 46 already-complete architectures listed in the superseded entry below, plus `llama3`, `mistral_v3`, `phi2`, `yi`, `starcoder2` (all confirmed calling `Checkpoint::from_reader` + `load_checkpoint` this pass — previously listed as "not yet implemented", that description is now wrong for these 5), plus `swin` and `deit` (confirmed calling `Checkpoint::from_reader` + `load_from_checkpoint` via a real `load_pretrained_report`, with dedicated tests per architecture-specific tensor-naming rule — previously listed as having "no weight-loading path at all", also now wrong). Plus, separately, the 8 always-on bonus architectures load/construct correctly.
- **NOT yet implemented (2/55)**: `llama3_2` (Mllama tile/aspect-ratio embeddings and gated cross-attention not modelled) and `deepseek` (fused MLA projections + `kv_a_layernorm` not modelled) — each returns a `not_implemented` error naming its specific architectural reason, which is a deliberate honest design choice given the current architecture, not an oversight.
- **No weight-loading path at all (0/55)**: none — this category is now empty; every feature-gated architecture at least implements `trustformers_core::traits::Model`.
...
- **Weight-loading gaps, updated 2026-08-18**: only 2 of 55 feature-gated architectures return a handled error instead of loading real checkpoints — `llama3_2` and `deepseek`, each for a specific, documented architectural reason (see "Per-Model Weight Loading Status" above). The other 5 this line used to list (`llama3`, `mistral_v3`, `phi2`, `yi`, `starcoder2`) now load real checkpoints, and `swin`/`deit` now have a full loading path — neither "no loading path" case remains.
...
**Status:** Alpha
**Model Count:** 55 feature-gated architectures + 8 always-on architectures, 53/55 with complete weight loading (`llama3_2` and `deepseek` honestly refuse with a named architectural reason); legacy Qwen2 deleted
