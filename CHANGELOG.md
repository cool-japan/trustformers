...
A production-grade honesty and correctness pass: replacing fabricated/placeholder logic with real implementations or structured errors, deleting unsound and orphaned code, fixing several deadlocks, and tightening dependency hygiene. Verified against the tree on 2026-08-18: `cargo check --workspace --all-targets` green; targeted `cargo nextest run -p <crate>` re-checks this pass show `trustformers-core` 3390/3390, `trustformers-models` 1681/1681 passing (workspace test count grew from 14,887 to 20,629+ passing across the underlying waves). Not everything targeted this cycle landed cleanly — see `TODO.md` for the specific items still open, including one currently-failing hygiene test and 7 open `cargo deny check advisories` findings introduced by this cycle's own dependency cleanup.
...
- Removing the four Azure SDK crates from `trustformers-serve` (see Changed, above) left 7 now-unreferenced entries in root `Cargo.toml`'s `[workspace.dependencies]` (`azure_core`, `azure_identity`, `azure_mgmt_machinelearningservices`, `azure_mgmt_web`, `google-cloud-functions-v2`, `google-cloud-gax`, `lambda_runtime`), which fails the `workspace_dependency_table_has_no_unused_entries` hygiene gate added in a prior cycle. Root-manifest cleanup is still owed.
- `cargo deny check advisories` currently fails with 7 findings (an unmaintained `opentelemetry-jaeger` and `paste`, an `h2` vulnerability, an RSA timing side-channel, two `rustls` name-constraints vulnerabilities, and a `rustls-webpki` CRL-parsing panic), all reached through the AWS SDK's `rustls 0.21`/`rustls-webpki 0.101.7` stack.
