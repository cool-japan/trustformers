...
pub struct ConflictDetector {
    ...
    _detection_rules: Arc<RwLock<Vec<ConflictDetectionRule>>>,
    /// Detected conflicts cache
    _detected_conflicts: Arc<RwLock<HashMap<String, DetectedConflict>>>,
    /// Conflict resolution strategies
    _resolution_strategies: Arc<RwLock<HashMap<ConflictType, Vec<ConflictResolutionStrategy>>>>,
    ...
    _learned_patterns: Arc<RwLock<Vec<LearnedConflictPattern>>>,
}
impl ConflictDetector {
    ...
    pub fn with_config(config: ConflictDetectionConfig) -> Self {
        Self {
            ...
            _detection_rules: Arc::new(RwLock::new(Self::create_default_rules())),
            _detected_conflicts: Arc::new(RwLock::new(HashMap::new())),
            _resolution_strategies: Arc::new(RwLock::new(Self::create_default_strategies())),
        ...
        if combined_memory_usage > memory_threshold {
            let conflict = DetectedConflict {
                ...
                conflict_info: ResourceConflict {
                    ...
                    conflict_type: ConflictType::CapacityLimit,
                    severity: self
                        .calculate_memory_conflict_severity(
                ...
                detection_details: ConflictDetectionDetails {
                    ...
                    evidence: vec![
                        ConflictEvidence { evidence_type :
                        ConflictEvidenceType::ResourceOverlap, description :
                        "Memory usage exceeds capacity threshold".to_string(), strength :
                        (combined_memory_usage - memory_threshold) / memory_threshold,
                        data : [("test1_usage".to_string(), format!("{} MB", usage1
                        .memory_mb)), ("test2_usage".to_string(), format!("{} MB", usage2
                        .memory_mb)), ("combined_usage".to_string(), format!("{:.2} GB",
                        combined_memory_usage)), ("threshold".to_string(),
                        format!("{:.2} GB", memory_threshold)),].iter().cloned()
                        .collect(), }
                    ],
                    false_positive_probability: 0.15,
                },
                resolution_options: self
                    .generate_memory_resolution_options(usage1, usage2),
                impact_analysis: self
                    .analyze_memory_conflict_impact(
                        combined_memory_usage,
                        memory_threshold,
                    ),
                detected_at: Utc::now(),
            ...
            let conflict = DetectedConflict {
                ...
                detection_details: ConflictDetectionDetails {
                    ...
                    evidence: vec![
                        ConflictEvidence { evidence_type :
                        ConflictEvidenceType::ResourceOverlap, description :
                        "Overlapping GPU device requirements".to_string(), strength :
                        overlap.len() as f32 / gpu1.len().max(gpu2.len()) as f32, data :
