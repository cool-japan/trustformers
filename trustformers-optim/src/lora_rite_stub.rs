    ...
    pub fn step(
        ...
        for (param_name, gradient) in gradients.iter() {
            if let Some(parameter) = parameters.get_mut(param_name) {
                ...
                let m_buffer =
                    self.state.m_buffers.get_mut(param_name).ok_or_else(|| {
                        anyhow::anyhow!("m_buffer should exist after initialization")
                    })?;
                let v_buffer =
                    self.state.v_buffers.get_mut(param_name).ok_or_else(|| {
                        anyhow::anyhow!("v_buffer should exist after initialization")
                    })?;
