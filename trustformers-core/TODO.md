...
- **New, 2026-08-24**: `gpu_ops/metal/types.rs` gained a real RAII `MetalBufferHandle` (refcounted; releases its cache entry on last-drop) and `Tensor::Metal` now stores one instead of a bare `BufferId` — see "Metal Backend" above. `trustformers-models/src/gpt2/model/{model_blocks.rs,model_core.rs,model_ops.rs}` has not been converted to match: `cargo check -p trustformers-models --features metal,gpt2` fails with 15 compiler errors today (5 `MetalTensorData{buffer_id:...}` construction-literal sites, 10 bare `.buffer_id` field reads). Not part of the default-feature workspace build. See root `TODO.md` P0 for the precise error list.
